...
@Service
public class AddScoreByDepositAmountActionHandler implements ActionHandler {
    ...
    public JSONObject handle(JSONObject context) {
        ...
        if(extraParams != null){
            ...
            if(investment != null){
                ...
                if(StringUtils.isNotBlank(userContent.getString("invitedByUser"))){
                    String invitedByUserId = userContent.getString("invitedByUser");
                    User invitedUser = userRepository.findById(invitedByUserId, false);
                    if(invitedUser != null){
                        TaskCreateRequest taskCreateRequest = new TaskCreateRequest();
                        JSONObject extraRequest = new JSONObject();
                        extraRequest.put("byUser", user);
                        extraRequest.put("investment", investment);
                        taskCreateRequest.setExtraParams(extraRequest);
                        taskCreateRequest.setInnerUserId(invitedByUserId);
                        taskCreateRequest.setApp(context.getString("appToken"));
                        taskCreateRequest.setTemplateCode("launchPadParticipantByInvited");
                        RoleDTO operator = new RoleDTO();
                        operator.setId(invitedByUserId);
                        operator.setName(invitedUser.getName());
